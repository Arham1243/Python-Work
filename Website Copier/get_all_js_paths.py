...
def extract_image_paths(url):
    try:
        ...
        js_scripts_paths = [
            script.get("src") for script in js_scripts if script.get("src")
        ]
        print(f"Scripts Found: {len(js_scripts_paths)}  Url: {url}")
        return js_scripts_paths
...
with open("js_scripts.py", "a") as f:
